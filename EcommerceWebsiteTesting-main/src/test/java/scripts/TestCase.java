...
public class TestCase {
	...
	@AfterMethod
	public void closeTest(ITestResult result) throws IOException, InterruptedException {
...
// Change the condition , If the screenshot needs to be taken for other status as well
		...
		((driver.findElements(By.xpath("//div[text()=\"Logout\"]")).size()) > 0) {
			action.clickLink(homePageObjects.lnkLogout, "logout link");
//loginPage.logoutBtn.click();
	...
	public void tc_1() throws Exception {
		try {
			...
			List<WebElement> alllink = (List<WebElement>) driver.findElement(By.tagName("a"));
			System.out.println(alllink.size());

			for (WebElement webElements : alllink) {
...
//		  Thread.sleep(3000);
	...
	@Test(groups = {"smoke"}, description = "TC007-This test verifies that user can login and logout with valid credentials present in config file", priority = 7, enabled = true)
	public void tc_7() throws Exception {
		try {

			extentReport.createTest("TC007-This test verifies that user can login and logout with valid credentials present in config file");
			action.clickLink(homePageObjects.lnkLogin, "Login link");
			extentReport.info("Login link clicked");
			action.sendKeys(homePageObjects.txtUserName, ConfigFileReader.getUserName() , "User Name");
			extentReport.info("User name entered");
		    action.sendKeys(homePageObjects.txtUserPass, ConfigFileReader.getPassword(), "User password");
	        extentReport.info("User password entered");
            action.clickButton(homePageObjects.btnLogIn, "Log In");
			extentReport.info("Credentials entered and Login button clicked");

			if (homePageObjects.lnkLogout.isDisplayed() && homePageObjects.homeWelcomeMessage.isDisplayed()) {
				extentReport.pass("User is on Home page of the application");
				Logging.info("User is on Home page of the application");
				Logging.endTestCase();
			} else {
				extentReport.fail("Logout link is not displayed");
				extentReport.addScreenshot(driver);
	...
	@Test(description = "TC008-Test to check number of category of products and find all the products", priority = 8, enabled = true, groups = {"smoke", "home"})
	public void tc_8() throws Exception {
		try {

			extentReport.createTest("TC008-Test to check number of category of products and find all the products");
			action.clickLink(homePageObjects.lnkLogin, "Login link");
			extentReport.info("Login link clicked");
			homePageObjects.loginWithConfigCreds();
			extentReport.info("Credentials entered and Login button clicked");
			Logging.info("Credentials entered and Login button clicked");

			if (homePageObjects.lnkLogout.isDisplayed() && homePageObjects.homeWelcomeMessage.isDisplayed()) {
				extentReport.info("User is on Home Page of application");
				Logging.info("User is on Home Page of application");

				List<WebElement> category = driver.findElements(By.xpath("//div[@class= 'category-list']/div/a"));
				int actualNumberOfCategories = category.size();
				int expectedNumberOfCategories = 4;
				Assert.assertEquals(actualNumberOfCategories, expectedNumberOfCategories);

				extentReport.info("Correct number of categories are appearing on Home Page");
				Logging.info("Correct number of categories are appearing on Home Page");
				...
				expectedCategoryNames.add("All Products");
				expectedCategoryNames.add("Electronics");
				expectedCategoryNames.add("Kitchen Items");
				expectedCategoryNames.add("Sports");

				Assert.assertEquals(categoryNames, expectedCategoryNames);

				extentReport.pass("Expected categories are appearing on screen");
				Logging.info("Expected categories are appearing on screen");
				Logging.endTestCase();
			} else {
				extentReport.fail("Expected categories are not appearing on screen");
				extentReport.addScreenshot(driver);
				Logging.info("Expected categories are not appearing on screen");
				Logging.endTestCase();
			}
		} catch (Exception e) {
			System.out.println(e.getMessage());
	...
	@Test(description = "TC009-Test to get all the available links on Home Page", priority = 9, enabled = true, groups = {"smoke", "regression", "home"})
	public void tc_9() throws Exception {
		try {

			extentReport.createTest("TC009-Test to get all the available links on Home Page");
			action.clickLink(homePageObjects.lnkLogin, "Login link");
			extentReport.info("Login link clicked");
			homePageObjects.loginWithConfigCreds();
			extentReport.info("Credentials entered and Login button clicked");
			...
			if (homePageObjects.lnkLogout.isDisplayed() && homePageObjects.homeWelcomeMessage.isDisplayed()) {
				...
				System.out.println("Number of links available on Home Page: " + allLinksElements.size());

				for (WebElement element : allLinksElements) {
					System.out.println(element.getText() + "---------" + element.getAttribute("href"));
				}
				Assert.assertEquals(allLinksElements.size(), 6);
				extentReport.pass("All the expected links are appearing on screen");
				Logging.info("All the expected links are appearing on screen");
				Logging.endTestCase();
			} else {
				extentReport.fail("All the expected links are not appearing on screen");
				extentReport.addScreenshot(driver);
				Logging.info("All the expected links are not appearing on screen");
				Logging.endTestCase();
			}
		} catch (Exception e) {
			System.out.println(e.getMessage());
	...
	@Test(description = "TC010-This test verify user can order product from all 3 categories", priority = 10, enabled = true)
	public void tc_10() throws Exception {
		try {
			extentReport.createTest("TC010-This test verify user can order product from all 3 categories");
			homePageObjects.loginWithConfigCreds();
			extentReport.info("User logged in");
			action.clickLink(homePageObjects.lnkElectronics, "Electronics");
			extentReport.info("Electronics category clicked");
			...
			action.clickLink(homePageObjects.lnkSports, "Sports Items");
			extentReport.info("Sports Items category clicked");
			action.clickImage(homePageObjects.lnkProducts, "First product");
			Thread.sleep(3000);
			double priceOnDetailsPageP3 = Double.parseDouble(homePageObjects.lblProductPrice.getText());
			action.clickButton(homePageObjects.btnAddToCart, "Add To Cart");
			extentReport.info("Add to cart button clicked");
			Utility.waitForElementVisible(driver, homePageObjects.btnGoToCart, 30);
			...
			extentReport.info("Confirm Payment button clicked");
			double cartValue = Double.parseDouble(homePageObjects.lblTotalCartAmount.getText());
			if (homePageObjects.lblConfirmPayment.isDisplayed() && homePageObjects.lblConfirmPayment.getText().equalsIgnoreCase("Order Confirmed") && cartValue == priceOnDetailsPageP1 + priceOnDetailsPageP2 + priceOnDetailsPageP3) {
				extentReport.pass("Order placed successfully");
				extentReport.addScreenshot(driver);
				Logging.info("Order placed successfully");
				Logging.endTestCase();
